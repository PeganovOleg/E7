...
def message():
    ...
    if request.method == 'POST':
        if data.get('text'):
            ...
            return jsonify({'ok': True, 'message': 'Сообщение успешно создано % s ' % res.inserted_id}), 200
        else:
            return jsonify({'ok': False, 'message': 'Нужно ввести текст'}), 400
...
def add_tag_to_message(message_id):
    ...
    if request.method == 'POST':
        if data.get('text'):
            ...
            return jsonify({'ok': True, 'message': 'Тег успешно вставлен % s ' % res}), 200
        else:
            return jsonify({'ok': False, 'message': 'Нужно ввести текст'}), 400
...
def message_by_id(message_id):
    if request.method == 'GET':
            ...
            return jsonify({'ok': True, 'message': 'Сообщение найдено  % s ' % res}), 200
